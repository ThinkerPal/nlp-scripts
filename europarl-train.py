...
ROOT = "/project/data/europarl/results"
...
for i in range(4):
    ...
    for k in range(5):
        ...
        saveFile = open(ROOT + "/train_" + str(k+1) + "." + LANG_ARRAY[i] , 'w+', encoding='utf-8')
        for j in testIter:
            if testIter.index(j) != i:
                print("Adding the " + j + "th line into the training file")
                tmp = ROOT + "/test_" + str(testIter.index(j)+1) + "." + LANG_ARRAY[i] 
